    ...
    <div className={styles.slider}>
      {
        data?.map((src, index) => (
          <div key={index} className={styles.img_container}>
            <img style={{gridArea: `pic${index}`}} className={`${styles.img_cut} ${styles.img_animation}`} alt="gallery" src={src} />
          </div>
        ))
      }
...
const getPicsSrc = (picNumber, picNumberOnSlide) => {
  ...
  for (let i = 0; i < Math.ceil(picNumber / picNumberOnSlide); i++) {
    ...
    for (let j = 0; j < picNumberOnSlide; j++) {
      scrs.push(`../public/gallery/gallery_${i * Math.ceil(picNumber / picNumberOnSlide) + j + 1}.jpg`);
    }
    data.push(scrs);
  }
  return data;
...
const Gallery = () => {
  ...
  const previousSlide = () => {
    ...
    currentSlide === 0 ? 
      setCurrentSlide(Math.ceil(picNumber / picNumberOnSlide) - 1) : setCurrentSlide(currentSlide - 1);
  };
  ...
  return (
    <section className={styles.container}>
      ...
      <span className={styles.carousel_text}>#ДАРИ<span className={styles.highlight}>&nbsp;с удовольствием</span></span>
    </section>
  );
};
